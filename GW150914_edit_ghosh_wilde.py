...
# Explenation: 1024 comes from 
#    1. finding the injected signal duration;  T=289.822 ...
#    2. setting a half-duration to be the smallest integer power of 2 > T; 512
#    3. multiplying the half-duration by 2 to get the full duration ; 1024

# Question 1: does ‘duration’ need to be a power of 2?
duration = 1024  

# Question 2: does ‘post_trigger_duration’ need to be a power of 2?
post_trigger_duration = 512  # Time between trigger time and end of segment
end_time = trigger_time + post_trigger_duration
start_time = end_time - duration

# Question 3: does ‘psd_duration’ need to be a power of 2?
...
for det in ["H1", "L1"]:
    ...
    data = TimeSeries.read(  file name, 
                           "{}:SIM-STRAIN".format(det), 
                           start_time, end_time)
    ifo.strain_data.set_from_gwpy_timeseries(data)

    logger.info("Downloading psd data for ifo {}".format(det))
    psd_data = TimeSeries.read(  file name, 
                           "{}:SIM-STRAIN".format(det) , 
